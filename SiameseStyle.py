...
class SiameseStyle(QueryByVoiceModel):
    ...
    def construct_representation(self, audio_list, sampling_rates, is_query):
        ...
        if is_query:
            ...
            return self._construct_representation_database(
    ...
    def predict(self, query, database):
        ...
        query = np.repeat(np.array(query), len(database), axis=0)

        # add another dimension to each
        # TODO: why?
        query = np.expand_dims(query, axis=1)
        database = np.expand_dims(np.array(database), axis=1)

        # run model inference
        model_output = self.model.predict(
            [query, database],  batch_size=1, verbose=1)
    ...
    def _construct_representation_query(self, query, sampling_rate):
        ...
        return [normalize(logmelspec).astype('float32')]

    def _construct_representation_database(self, database, sampling_rates):
        new_sampling_rate = 44100
        spectrograms = []
        for audio, sampling_rate in zip(database, sampling_rates):
            ...
            # normalize to zero mean and unit variance
            normed = normalize(logmelspec).astype('float32')
            spectrograms.append(normed)

        return spectrograms

    def self._normalize(x):
        # normalize to zero mean and unit variance
