...
def search_audio(imi_path, ref_dir, model_path, text):
    ...
    i_delete = []
    text_match_data = []
    text_match_filenames = []
    for i, fn in enumerate(old_ref_filenames):
        if text in fn.lower():
            i_delete.append(i)
            text_match_filenames.append(fn)
            text_match_data.append(old_ref_data[i])

    ref_filenames = np.delete(old_ref_filenames, i_delete,0)
    ref_data = np.delete(old_ref_data, i_delete,0)
    sorted_filenames = inference(ref_data, ref_filenames, imi_data, model)

    if len(text_match_filenames) != 0:
        sorted_filenames_matched = inference(np.array(text_match_data), np.array(text_match_filenames), imi_data, model)
    else: 
        sorted_filenames_matched = np.array([])
