...
class TestSiameseStyle(object):
    ...
    def setUp(self):
        ...
        dataset_directory = os.path.realpath('../data/test_dataset')
    ...
    def test_construct_representation(self):
        dataset = self.model.construct_representation(
            [self.cat, self.dog], [self.cat_sr, self.dog_sr], is_query=False)
        # The number of representations should match the number of audio clips
        self.assertEqual(len(dataset), 2)
        for datum in dataset:
            # The representations should be normalized
            self.assertTrue(math.isclose(datum.mean(), 0.0))
            self.assertTrue(math.isclose(datum.std(), 1.0))

        query = self.model.construct_representation(
            [self.cat], [self.cat_sr], is_query=True)
        # The number of representations should match the number of audio clips
        self.assertEqual(len(query), 1)
        # The representations should be normalized
        self.assertTrue(math.isclose(query[0].mean(), 0.0))
        self.assertTrue(math.isclose(query[0].std(), 1.0))
