...
class OtoMobile(QueryByVoiceDataset):
    '''
    ...
    def __init__(self,
        ...
        self.binary_filename = os.path.join(
            representation_directory, 'otomobile.pickle')
        self.data_dict = None
        self.csv = pd.read_csv(
            os.path.join(dataset_directory, 'otomobile.csv'))

        super(OtoMobile, self).__init__(
            dataset_directory,
            representation_directory,
            model,
    ...
    def handle_to_filename(self, handle):
        ...
        '''
        return os.path.join('dataset', handle)
    ...
    def _get_audio_filenames(self):
        ...
        Returns:
            A python list.
        '''
        path_pairs = zip(self.csv['diagnosis'], self.csv['filename'])
        return [os.path.join('dataset', d, f) for (d, f) in path_pairs]
    ...
    def _get_representation_handles(self):
        ...
        '''
        path_pairs = zip(self.csv['diagnosis'], self.csv['filename'])
        return [os.path.join(d, f) for (d, f) in path_pairs]
    ...
    def _load_representations(self, handles):
        ...
        return [self.data_dict[os.path.basename(h)] for h in handles]
        ...
        Arguments:
            ...
            filenames:
                A python list. The corresponding list of audio filenames (i.e.,
                    ...
                    filenames[i]).
        '''
        pairs = zip(filenames, representations)
        self.data_dict = {os.path.basename(f): r for (f, r) in pairs}
